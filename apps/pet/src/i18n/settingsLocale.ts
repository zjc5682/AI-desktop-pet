...
export const SETTINGS_EN_TEXT = {
  ...
  voiceProvidersCopy:
    'Current runtime supports optional Silero VAD, local faster-whisper path loading, Whisper or VibeVoice ASR, and cloud or offline TTS providers.',
  vad: 'VAD',
  energyFallback: 'energy fallback',
  stt: 'STT',
  tts: 'TTS',
  sttModelPath: 'STT model path',
  sttModelPathHelp:
    'For faster-whisper, point this to a local CTranslate2 model directory. If left empty, the backend only searches local caches and never downloads models online.',
  sttModelSize: 'STT local model preset',
  sttModelSizeHelp:
    'Used only as a local cache lookup hint when STT model path is empty.',
  speechLanguage: 'Speech language',
  ttsVoice: 'TTS voice',
  offlineTtsFallbackHelp:
    'Edge-TTS now falls back to local offline Windows system speech when the online provider returns no audio. Piper currently uses the same offline fallback path.',
  ...
  previewEmpathyTitle: 'Emotion sync',
  previewEmpathyCopy:
    'Keep Live2D expression changes aligned with camera and voice analysis without showing a floating camera preview.',
...
export const SETTINGS_ZH_TEXT: Partial<Record<SettingsLocaleKey, string>> = {
  ...
  voiceProvidersCopy:
    '当前运行时支持可选的 Silero VAD、本地 faster-whisper 模型路径、Whisper 或 VibeVoice ASR，以及云端或离线 TTS Provider。',
  energyFallback: '能量回退',
  sttModelPath: 'STT 模型路径',
  sttModelPathHelp:
    '使用 faster-whisper 时，这里填写本地 CTranslate2 模型目录。留空时后端只会查找本机缓存，不会再联网下载模型。',
  sttModelSize: 'STT 本地模型预设',
  sttModelSizeHelp:
    '当 STT 模型路径留空时，这个值只用于本地缓存查找提示。',
  speechLanguage: '语音语言',
  ttsVoice: 'TTS 音色',
  offlineTtsFallbackHelp:
    'Edge-TTS 在在线返回空音频时会自动回退到本地离线 Windows 系统语音。当前 Piper 也复用这条离线回退链路。',
...
SETTINGS_ZH_TEXT.previewEmpathyTitle = '情感同步';
SETTINGS_ZH_TEXT.previewEmpathyCopy =
  '让 Live2D 表情跟随摄像头和语音分析结果同步变化，不再显示悬浮相机预览。';
SETTINGS_ZH_TEXT.vibeVoiceRealtimeUrlPlaceholder =
  'http://127.0.0.1:3000 或 ws://127.0.0.1:3000/stream';
SETTINGS_ZH_TEXT.vibeVoiceRealtimeHelp =
  '如果你使用的是官方 VibeVoice realtime demo，可以填写根 HTTP 地址或 /stream websocket 地址，运行时会自动转成官方协议形式。';
SETTINGS_ZH_TEXT.vibeVoiceVoiceHelp =
  '当 TTS Provider 选择 VibeVoice-Realtime 时，TTS 音色字段会被当作 VibeVoice 的 voice preset key，例如 en-Carter_man。';
